...
При установке на новую машину (нет существующего `server-config.json`) пункт "Install server" теперь задаёт всего 4 вопроса - listen prefix/порт, открывать ли Windows Firewall, логин и пароль дашборда - и печатает после установки сводку того, что осталось по умолчанию (HTTPS, AD sync, токен приёма данных, GPO-пакет клиента) и где это донастроить - HTTPS/AD sync через Settings, GPO-пакет через вкладку Client package. Сам автосгенерированный токен нигде в дашборде не показывается - если клиентам нужен токен, который можно им дать, переустановите с `-Token <значение>` (или выберите "Full reconfigure"). Уже установленный сервер по-прежнему получает существующий выбор "Just refresh" / "Full reconfigure" без изменений.
...
Обе кнопки ищут по тем Organizational Units, что указаны в блоке «Active Directory» на странице Settings > General («Organizational Units (DN, one per line)» - один Distinguished Name на строку, например `OU=Workstations,OU=Site1,DC=corp,DC=example,DC=com`), включая всё вложенное внутрь каждой из них. Если список пуст, поиск идёт по всему домену. Обе используют те же домен и учётные данные AD, что уже настроены в блоке «Configure AD User» выше - если он уже настроен, донастраивать нечего.
