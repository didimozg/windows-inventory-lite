...
- Навигация web-интерфейса — древовидная боковая панель, зафиксированная на месте при прокрутке содержимого: Dashboard, Windows Inventory (Clients, Software, Hardware), Linux Inventory (Clients, Software, Hardware), Licenses, Installation (Client actions, Client package), Settings (General, Certificate, Change admin password). Плашка с версией сервера — внизу боковой панели.
- Боковая панель дашборда теперь делит инвентарь на две верхнеуровневые группы: **Windows Inventory** (Clients/Software/Hardware, как раньше) и **Linux Inventory** (Clients/Software/Hardware, зеркально той же структуре - пагинация, сортировка, раскрытие строки с деталями, поиск, экспорт в CSV и живое обновление каждые 30 секунд). Linux Software группирует по имени пакета и версии (у Linux-пакетов нет издателя и даты установки); Linux Hardware группирует CPU/накопители/RAM так же, как и Windows-таблицы, используя те данные, что уже собирает Linux-клиент (без тактовой частоты, без определения USB-дисков, без разбивки RAM по модулям на этой стороне).
...
- Вкладка Client package показывает версии клиентских исполняемых файлов и текущие настройки CMD-файла, позволяет обновить адрес сервера, токен приема, интервал отчетности и путь до сетевой папки пакета, а также скачать готовый GPO-пакет в виде ZIP-архива.
...
После установки сервера адрес сервера, токен, интервал отчетности и путь до сетевой папки пакета в `Install-ClientGpo.cmd` можно изменить через вкладку `Client package` в web-интерфейсе без повторной сборки. Там же можно скачать готовый ZIP-пакет.
...
Если сервер уже установлен, пункт "Install server" предложит вариант "Just refresh" — он пропускает все вопросы и переприменяет текущие сохранённые настройки как есть (удобно после обновления сборки). Чтобы изменить какую-то настройку, выберите "Full reconfigure".

При установке на новую машину (нет существующего `server-config.json`) пункт "Install server" теперь задаёт всего 4 вопроса - listen prefix/порт, открывать ли Windows Firewall, логин и пароль дашборда - и печатает после установки сводку того, что осталось по умолчанию (HTTPS, AD sync, токен приёма данных, GPO-пакет клиента) и где это донастроить - HTTPS/AD sync через Settings, GPO-пакет через вкладку Client package. Само значение автосгенерированного токена при установке не показывается нигде; проверить, задан ли он, и перегенерировать его можно позже через Settings > General (см. ниже), либо сразу переустановить с `-Token <значение>` (или выбрать "Full reconfigure"), чтобы задать конкретное значение сейчас же. Уже установленный сервер по-прежнему получает существующий выбор "Just refresh" / "Full reconfigure" без изменений.

Токен приёма данных, которым защищены `POST`-запросы к `/api/v1/inventory` и `/api/v1/linux/inventory`, теперь действительно генерируется автоматически, если `Install-Server.ps1` запускается без явного `-Token` и без ранее сохранённого значения - это всегда было заявленным поведением, но реально никогда не реализовывалось: пустой токен означал полностью незащищённый приём данных. Новый раздел «Ingestion Token» в Settings > General показывает, задан ли токен, и позволяет его перегенерировать - как и на странице Admin Password, текущее значение нигде не показывается, оно отображается один раз, сразу после перегенерации, поскольку это единственный момент, когда оно известно в открытом виде. Перегенерация токена ломает приём данных от всех уже установленных клиентов, пока каждый из них не будет переустановлен или перенастроен с новым значением.

Пункт "Install client (local)" теперь тоже предлагает выбор "Just refresh"/"Full reconfigure", если клиент уже установлен - мастер читает командную строку уже работающей службы и восстанавливает из неё URL сервера, путь к share, токен, интервал и путь установки, так что "Just refresh" переустанавливает с теми же настройками без единого вопроса. Это работает для клиента, поставленного любым способом (этим мастером, push через WinRM или GPO-пакетом) - читается сама служба, а не файл настроек, привязанный к одному конкретному способу установки. Если командную строку не удаётся разобрать (не распознан или отсутствует URL сервера), мастер молча переходит к обычным вопросам, как если бы клиент не был установлен вовсе.
...
Сервер хранит логи заданий WinRM в `DataPath\_client-install-jobs`. Срок хранения по умолчанию составляет 30 дней. Начальное значение можно задать при установке сервера:
...
Изменить его позже можно в Settings > General («Client action log retention (days)», в блоке Inventory) - применяется ко всем заданиям установки/обновления/удаления, текущим и будущим, а не только к тому, с которым устанавливалось. В сохраненный лог попадают действие, цели, статус, вывод команды, ошибки, временные метки и имя пользователя WinRM. Пароли в файлы логов не записываются.

Ошибка подключения показывается одним из двух коротких сообщений - не удалось разрешить имя компьютера, либо служба WinRM недоступна на этом компьютере - с добавлением исходного текста ошибки, вместо необработанного текста исключения на языке ОС.
...
Опциональная функция, по умолчанию выключена. Блок «Active Directory» на странице Settings > General разделён на два независимых чекбокса:

- **Configure AD User** - делает домен и учётные данные AD ниже доступными для `Client actions`, `Client updates` и импорта компьютеров из AD (см. ниже). Сам по себе этот чекбокс не затрагивает данные инвентаризации.
- **Sync Description from AD** - подтягивает атрибут `description` из Active Directory для каждого отчитывающегося компьютера и записывает его в колонку Description на вкладке `Clients`. Пока эта опция включена, колонка доступна только для чтения, а её заголовок — «AD Description». Выключите синхронизацию — заголовок сменится на обычный «Description», а колонку можно будет редактировать прямо в web-интерфейсе: введённое вручную значение остаётся, пока синхронизацию не включат обратно, после чего ближайшее обновление снова перезапишет его значением из AD.

Включить AD identity при установке:

```powershell
.\src\Install-Server.ps1 -AdSyncEnabled
```

`-AdSyncEnabled` настраивает только identity. При первой установке у синхронизации Description ещё нет собственного сохранённого значения, поэтому она наследует то же состояние, что задано в `-AdSyncEnabled` — первая установка ведёт себя точно так же, как раньше вёл себя единый флаг. Инсталляции, обновляемые с версии до разделения флага, продолжают синхронизировать Description без каких-либо действий администратора — по той же причине. После установки оба параметра переключаются независимо друг от друга со страницы Settings > General.

Два режима синхронизации Description:
...
Если имени компьютера не находится соответствующий объект в AD, колонка Description показывает «Not found in AD»; если AD была недоступна в момент синхронизации — «AD unreachable», а следующий отчёт или проход таймера повторит попытку, не дожидаясь окончания всего интервала синхронизации. Чтение атрибута description по умолчанию разрешено любому аутентифицированному пользователю домена стандартными AD ACL, так что учётной записи службы обычно не нужны отдельные делегированные права.

## Импорт списка компьютеров из AD

На вкладке `Client actions` две кнопки подтягивают список компьютеров прямо из Active Directory и подставляют его в поле Targets, заменяя то, что там было - быстрее, чем вводить имена вручную перед WinRM-установкой или удалением:

- **Load all PC from AD** - все компьютеры из настроенной области поиска.
- **Load PC without client from AD** - та же область, но отфильтрованная до компьютеров, на которых ещё нет отчитывающегося клиента (сверка со списком на вкладке `Clients`) - удобно для поиска целей под первичную установку без ручного вычёркивания уже установленных машин.

Обе кнопки ищут по тем Organizational Units, что указаны в блоке «Active Directory» на странице Settings > General («Organizational Units (DN, one per line)» - один Distinguished Name на строку, например `OU=Workstations,OU=Kaliningrad,DC=spb,DC=cccb,DC=ru`), включая всё вложенное внутрь каждой из них. Если список пуст, поиск идёт по всему домену. Обе используют те же домен и учётные данные AD, что уже настроены в блоке «Configure AD User» выше - если он уже настроен, донастраивать нечего.

«Load all PC from AD» возвращает сырой список компьютеров для указанной области поиска, без фильтрации по тому, отчитывался ли компьютер когда-либо серверу - при необходимости список можно поправить вручную (например, для удаления клиента обычно нужны только те компьютеры, где он реально установлен). Если одна из указанных OU не находится (опечатка, удалённая OU), она пропускается с предупреждением, а остальные всё равно загружаются - для обеих кнопок.
...
Вкладка `Client updates` в web-интерфейсе (раздел Installation) показывает, какие отчитывающиеся клиенты работают на версии, отличной от текущего клиентского пакета на сервере, и позволяет обновить любого из них по WinRM одной кнопкой — используя тот же механизм установки, что и `Client actions`.

По умолчанию обновление использует identity самой серверной службы — то же требование к WinRM, что уже действует для `Client actions`. Если этой identity недостаточно для доступа к целям, на странице `Client updates` можно сохранить отдельную учётную запись WinRM (`Client update username` / `Client update password`) — пароль шифруется так же, как `WebPassword`/`AdPassword`/`Token`. Параметра в `Install-Server.ps1` для этих учётных данных нет — они настраиваются только через web-интерфейс.

Вместо того чтобы сохранять отдельную учётную запись, можно отметить «Use global AD settings» на странице `Client updates` - тогда используются те же домен и учётные данные AD, что уже настроены в блоке «Configure AD User» (Settings > General > Active Directory): собственная учётная запись службы, если отмечено «Use service account identity», либо сохранённый AD-аккаунт. Для этого нужно, чтобы AD identity была настроена, а если используется явный аккаунт - чтобы для него были сохранены имя пользователя и пароль; иначе запуск отклоняется с понятным сообщением об ошибке.
...
Web-интерфейс опрашивает сервер каждые 30 секунд и обновляется на месте: новые или изменившиеся данные появляются без перезагрузки страницы и без потери текущего состояния (открытой страницы, сортировки, поискового фильтра, развернутых строк детализации). Опрос приостанавливается, пока вкладка браузера скрыта, и сразу подхватывает изменения, как только вкладка снова становится видимой.
...
- `Clients`: одна строка на компьютер, с ОС, Office, состоянием активации, количеством ПО, временем отчета, версией агента и колонкой Description (см. [Интеграция с Active Directory](#интеграция-с-active-directory)) - «AD Description» только для чтения, пока включена синхронизация Description из AD, либо обычное «Description», редактируемое прямо в таблице, пока она выключена. Компьютеры с USB-накопителями помечаются значком. Нажмите имя компьютера, чтобы открыть детализацию: сводку по оборудованию (CPU, ОЗУ, накопители) и полный список ПО.
...
- `Client package`: показывает версии клиентских `.exe` и текущие настройки `Install-ClientGpo.cmd`. Позволяет изменить адрес сервера, токен, интервал отчетности и путь до сетевой папки пакета, а также скачать готовый GPO-пакет в виде ZIP-архива.
...
| `-AdSyncEnabled` | `off` | Включить AD identity - домен/учётные данные для `Client actions`, `Client updates`, импорта компьютеров из AD и (по умолчанию, при первой установке) синхронизации Description (см. [Интеграция с Active Directory](#интеграция-с-active-directory)). |
| `-AdSyncMode` | `on-report` | Режим синхронизации Description: `on-report` или `timer`. |
| `-AdSyncIntervalHours` | `24` | Как часто обновляется Description компьютера из AD (1–8760). |
...
| `-CredentialPassword` | `—` | Пароль WinRM в виде `SecureString`. Используется, если не указан `-Credential`. |
...
- Пока Basic Auth не настроен, весь management API (web-интерфейс, настройки, импорт сертификата, WinRM-действия с клиентами, первоначальная настройка пароля) принимает запросы только с самого сервера, а не из сети. Чтобы выполнить первоначальную настройку удалённо, подключитесь к серверу по RDP или консоли для этого одного шага, либо передайте `-WebUsername`/`-WebPassword` в `Install-Server.ps1` при установке. После установки пароля его смена всегда требует текущий пароль, и доступ перестаёт ограничиваться локальной машиной.
...
- `serverUrl`, `token` и `packageSharePath` отклоняются, если содержат двойную кавычку, `&`, `|`, `<`, `>`, `^` или перевод строки — как на вкладке `Client package`, так и в `New-ClientGpoPackage.ps1`. `Install-ClientGpo.cmd` — это batch-файл, который сценарий запуска компьютера GPO выполняет от имени SYSTEM на каждом развёрнутом клиенте, и любой из этих символов мог бы превратить настроенное значение во внедрённую команду.
