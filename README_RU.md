...
Вместо ввода учётных данных WinRM для конкретного задания можно отметить «Use global AD settings» на вкладке `Client actions` - тогда используются те же домен и учётные данные AD, что уже настроены для синхронизации Description (Settings > General > Active Directory): собственная учётная запись службы, если отмечено «Use service account identity», либо сохранённый AD-аккаунт. Для этого нужно, чтобы синхронизация AD была включена, а если используется явный аккаунт - чтобы для него были сохранены имя пользователя и пароль; иначе запуск отклоняется с понятным сообщением об ошибке.
