...
Сервер ищет TLS-сертификат в хранилище сертификатов Windows `LocalMachine\My` по отпечатку. Встроенного связывания через HttpListener/netsh нет — сервер сам оборачивает каждое принятое соединение в `SslStream`, поэтому включение или замена сертификата вступает в силу без перезапуска службы, как только сертификат зарегистрирован.
...
- `Software`: одна строка на имя ПО, версию и издателя, с количеством установок и списком компьютеров. Колонка License ведёт к соответствующей записи лицензии, если она уже заведена для этого названия ПО — одна лицензия обычно распространяется на несколько установленных версий, поэтому сравнение идёт только по названию, без версии.
...
- `General`: порог «устаревания» (в часах) и переключатель HTTPS. Включение HTTPS заново проверяет настроенный сертификат на риски и просит подтверждения, если что-то найдено.
