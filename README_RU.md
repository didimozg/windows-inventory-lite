...
либо развернуть Linux-клиент по SSH, используя приватный ключ RSA, Ed25519 или ECDSA в формате OpenSSH:
...
Дашборд - однастраничное приложение с горизонтальной панелью сверху: **Dashboard** (сводные плитки и графики по ОС и оборудованию для обоих парков), **Fleet** (выпадающий список: Clients, Software (Windows), Services (Linux), Hardware - Clients и Hardware объединяют обе платформы за фильтром All/Windows/Linux), **Software** (отдельный выпадающий список каталогов Windows Updates и Third-Party Software - это пакеты, которые дашборд может поставить на клиенты, а не инвентаризация уже установленного, за это отвечает Fleet), **Install** (вкладки Actions, Updates, Package - установка и удаление по WinRM/SSH с режимом автоопределения, обновление устаревших клиентов, скачивание автономного пакета установки), **Licenses** (отдельный верхнеуровневый каталог лицензионных ключей с подвкладками Catalog, Sources, Keys), **Settings** (Server, Windows, Linux, Certificate, Admin password) и **Logging** (четыре подвкладки - Ingestion Rejections, Installs, Software и текстовый просмотрщик Debug-лога, у каждой своя кнопка "Clear log"). Вход теперь идёт через отдельную страницу логина и настоящую серверную сессию, а не через нативный диалог Basic Auth браузера - сам Basic Auth (`curl -u user:pass`) по-прежнему работает без изменений для скриптов и автоматизации.
