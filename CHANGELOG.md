...
- **Client-pulled software distribution**: two new admin-managed catalogs, Windows Updates (`/api/v1/windows-updates*`) and Third-Party Software (`/api/v1/third-party-software*`), each with its own dashboard tab under a new top-level "Software" nav dropdown alongside a shared "Attempt history" tab. Every catalog entry (`{id, name, relativePath, arguments, requiresReboot, enabled, targetComputers, targetGroups}`) is targeted at computers/groups the same way a Deploy > Actions job already is.
