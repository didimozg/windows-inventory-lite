...
- **Client-pulled software distribution**: two new admin-managed catalogs, Windows Updates (`/api/v1/windows-updates*`) and Third-Party Software (`/api/v1/third-party-software*`), each with its own dashboard tab under a new top-level "Software" nav dropdown alongside a shared "Attempt history" tab. Every catalog entry (`{id, name, relativePath, arguments, requiresReboot, enabled, targets}`) is targeted the same way a Deploy > Actions job already is - `targets` is the same plain delimited computer-name/IP-range text `ExpandInstallTargets` already parses there; there is no separate server-stored "group" entity.
- **Network-share pull, independent of both the existing WinRM/SSH push and the inventory report cycle**: the client fetches its assigned jobs from a new ingestion-token-gated `GET /api/v1/client/software-jobs` endpoint on its own client-configurable check interval - a separate timer from the existing inventory collection loop, not piggybacked on `POST /api/v1/inventory` the way the license-key-source catalog is. The server never connects outbound to a client for this feature, the opposite direction from Deploy > Actions/Updates' WinRM/SSH push; the actual installer/patch files live on an admin-configured UNC share (`SoftwareRepositoryPath`) that the client reads directly, and the client keeps a local success-state cache of entry IDs it has already run so a later cycle does not repeat them (a failed entry is retried every cycle until it succeeds or the admin disables/removes it).
...
215 self-tests (was 203), 133/133 Pester green under Windows PowerShell 5.1.
