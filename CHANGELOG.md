...
- Dashboard static files (`app.js`, `index.html`, `styles.css`) are now served with `Cache-Control: no-cache`, so a browser revalidates before using a cached copy instead of potentially showing a stale dashboard build after a server update until a hard refresh.
