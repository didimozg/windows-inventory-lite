...
- Shared token used by clients in `X-Inventory-Token`. Whether it is enforced is an explicit `RequireIngestionToken` setting since 0.29.0 (previously an implicit side effect of whether a token happened to be configured at all) - see Main Risks and Controls below. The value itself is intentionally readable in plaintext by any authenticated dashboard session (`GET /api/v1/server/ingestion-token`); it is treated as low-sensitivity, since a leak only lets someone submit fake inventory data, not gain any broader access.
...
- POST body to `POST /api/v1/server/settings` (staleHours, useHttps, port, enableHttp, httpsPort, acknowledgeRisks, requireIngestionToken, acknowledgeIngestionTokenRisk fields, among others) that changes the stale threshold, independently starts, stops, or moves the HTTP and HTTPS listeners using the currently configured certificate, and toggles whether `/api/v1/inventory`/`/api/v1/linux/inventory` require a matching token at all.
...
- Disabling `RequireIngestionToken` from `POST /api/v1/server/settings` must require an explicit `acknowledgeIngestionTokenRisk` flag, evaluated before any other change in the same request (listener rebinds, AD settings) is applied to live server state - so a rejected request never leaves live state ahead of what is persisted to disk. This flag is independent of the certificate risk gate's own `acknowledgeRisks`, so acknowledging one can never silently bypass the other when both are toggled in the same save.
...
- If a browser has cached Basic Auth credentials for the dashboard's origin, a malicious page could trigger a cross-site request the browser auto-authenticates (a CSRF-like risk inherent to Basic Auth, not specific to this project). Mitigated in v0.48.0: every state-changing route (POST/PUT/DELETE, except the two device-ingestion endpoints, which authenticate via a bearer-style token a browser never attaches automatically and so are not exposed to this pattern) now rejects the request if `Origin` or `Referer` is present and doesn't match the server's own host, and rejects any request carrying a body unless `Content-Type: application/json` is set - a plain HTML form (the classic zero-JS CSRF vector) cannot set that header, and a form using `enctype="text/plain"` to smuggle a JSON-shaped body is caught by the same check. Both headers are checked only when present, never required, so direct API automation (curl, Postman, the examples in `docs/api-reference.md`) that sends neither is unaffected - there is no browser being tricked in that case. The admin password change endpoint's current-password requirement remains a second, independent factor for that one action. The Origin/Host comparison itself trusts the raw `Host` header as sent, not a value derived through a reverse proxy - not a gap for this server's actual deployment model (it terminates HTTP(S) itself, no proxy in front by default), but worth revisiting if that ever changes. An attacker who has directly stolen valid Basic Auth credentials (as opposed to riding a victim's browser) can trivially craft a request with a matching `Host`/`Origin` pair and pass this check - that's expected, not a bypass of anything: a credentialed attacker doesn't need to defeat CSRF at all, since omitting both headers already passes by design (see above), and having real credentials makes them an authenticated caller, not the tricked-victim scenario this control exists for.
- Mitigated in v0.50.0: failed Basic Auth attempts are now rate-limited per source IP (`IsBasicAuthLockedOut`/`RecordBasicAuthAttempt`) - after `LoginLockoutThreshold` (default 10) wrong-credential attempts from the same IP within `LoginLockoutWindowMinutes` (default 15), that IP gets `429 Too Many Requests` for `LoginLockoutDurationMinutes` (default 15), including on a subsequent *correct*-password attempt. Tracked per source IP rather than account-wide specifically so one attacker cannot lock out the legitimate admin, who is presumed to connect from a different address - the same trust boundary the network-level control below already assumes, not a new gap. `FixedTimeEquals` still closes the timing side-channel independently of this. In-memory only (a server restart clears all tracked IPs) and does not extend an active lockout on further attempts (a sustained flood cannot keep an IP locked out indefinitely) - both deliberate, see `docs/superpowers/specs/2026-08-25-basic-auth-rate-limiting-design.md`. The documented primary control remains network-level restriction (deploy on a trusted management network, not exposed to the internet); this is defense-in-depth on top of it, not a replacement. Configurable via Settings > Admin password > Login lockout, or disable entirely with `LoginLockoutThreshold: 0` (e.g. for a test bench doing rapid scripted auth checks).
...
- Since 0.29.0, ingestion token enforcement is an explicit, admin-controlled on/off setting (`RequireIngestionToken`), not an incidental side effect of whether a token string happens to be configured. Turning it off is a deliberate, risk-acknowledged action - while off, both `/api/v1/inventory` and `/api/v1/linux/inventory` accept any request unauthenticated, from anyone who can reach the port. The token value is always readable in plaintext by an authenticated dashboard session (`GET /api/v1/server/ingestion-token`) - a deliberate posture, not an oversight: the token is treated as low-sensitivity (fake inventory submission, not broader access), so read exposure to an already-authenticated admin session is an accepted trade-off for being able to see and copy the value without regenerating it.
...
Day-to-day guidance that follows from the controls above, aimed at an operator running the server rather than someone auditing its design:
...
- The debug log can contain client-reported computer names and full exception text (including stack traces) but never passwords or tokens. Treat it like any other operational log - avoid leaving it enabled longer than a troubleshooting session needs.
...
- The Linux client's ingestion token is delivered through a mode-600 `/etc/wil-linux-client.env` file on the managed host, not on the systemd unit's command line - keep that file's permissions intact and do not copy its contents into a broader-permission location.
- SSH-key-mode pushes to a Linux host verify the target's host key against the fingerprint pinned when it was first trusted. A "host key changed" warning must never be dismissed without independently confirming the target was genuinely reimaged or rekeyed on purpose.

### Recovering from an HTTPS lockout

The HTTP-disable safety gate only checks listener state at the moment HTTP is disabled - it cannot see a certificate expiring, being deleted from the store by another tool, or otherwise breaking later. If that happens after HTTP was already turned off, the dashboard becomes unreachable, since there is no listener left to serve it. Recover with either of these, run locally on the server:
