...
- No rate limiting or lockout exists on failed Basic Auth attempts - an attacker with network access to the port can attempt passwords indefinitely at whatever rate the TCP/thread-per-connection model allows, with no throttling or account lockout. `FixedTimeEquals` (see below) closes the timing side-channel but does nothing to slow down raw guessing. The documented primary control is network-level restriction (deploy on a trusted management network, not exposed to the internet) rather than an in-app lockout, which would need careful design of its own (per-IP vs. global tracking, a lockout window that can't itself become a denial-of-service vector against the legitimate admin). Not yet implemented.
