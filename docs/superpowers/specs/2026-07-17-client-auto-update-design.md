...
- A credentials block above the table (plain `.settings-block`, same visual pattern as the `General` settings page's grouped fields - there is no collapsible/expandable UI pattern anywhere in this codebase to reuse, confirmed during plan research) with `Client update username` / `Client update password` fields and a Save button, plus a note that leaving both blank uses the service's own identity.
