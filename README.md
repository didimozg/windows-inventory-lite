...
The dashboard is a single-page app with a horizontal top bar: **Dashboard** (summary tiles and OS/hardware breakdown charts across both fleets), **Fleet** (a dropdown for Clients, Software (Windows), Services (Linux), and Hardware - Clients and Hardware merge both platforms behind an All/Windows/Linux filter), **Software** (a dropdown for the Windows Updates and Third-Party Software catalogs - packages the dashboard can push to clients, distinct from Fleet's own read-only inventory of what's already installed), **Install** (Actions, Updates, and Package tabs - install or uninstall over WinRM/SSH with an Auto-detect mode, push outdated clients, or download a self-contained install package), **Licenses** (its own top-level catalog of license keys, with Catalog, Sources, and Keys sub-tabs), **Settings** (Server, Windows, Linux, Certificate, and Admin password), and **Logging** (four sub-tabs - Ingestion Rejections, Installs, Software job attempts, and a raw Debug log viewer - each with its own "Clear log" button). Signing in uses a small dedicated login page and a real server-side session, not the browser's native Basic Auth prompt - Basic Auth (`curl -u user:pass`) still works unchanged for scripts and automation.
